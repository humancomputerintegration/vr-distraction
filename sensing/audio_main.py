...
import ubicoustics.ubicoustics as ubicoustics
import ubicoustics.microphones as microphones
from ubicoustics.vggish_input import waveform_to_examples

from datetime import datetime
from ubicoustics.reprint import output
from ubicoustics.helpers import Interpolator, ratio_to_db, dbFS, rangemap
...
def audio_samples(in_data, frame_count, time_info, status_flags):
    ...
    with graph.as_default():
        ...
        for prediction in predictions:
            ...
            candidate = (label[m], prediction[0, m])
    ...
    while stream.is_active():
        with output(initial_len=OUTPUT_LINES, interval=0) as output_lines:
            while True:
                ...
                if (
                    conf > PREDICTION_THRES
                    and db > DBLEVEL_THRES
                    and event in DETECTION_LIST
                ):
                    pred = event
                ...
                if count == 1:
                    stimulus_start = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")

                if count == 10:
                    f = open("detection.txt", "a")
                    f.write(stimulus_start)
                    f.write(" ")
                ...
                last_pred = pred
                output_lines[32] = "%20s: %s" % ("Prediction", pred)
